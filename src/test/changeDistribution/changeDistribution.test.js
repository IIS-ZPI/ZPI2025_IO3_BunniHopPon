...
describe("histogramDistribution module", () => {
  describe("calculateDailyChanges", () => {
    it("throws an error when input is not a valid array", () => {
      ...
      expect(() => calculateDailyChanges("invalid")).toThrow(
        "Insufficient points",
      );
    ...
    it("throws an error when input array has less than two points", () => {
      expect(() => calculateDailyChanges([{ rate: 4.0 }])).toThrow(
        "Insufficient points",
      );
    });
    ...
    it("throws an error on invalid or negative rates", () => {
      expect(() =>
        calculateDailyChanges([{ rate: 4.0 }, { rate: -4.0 }]),
      ).toThrow("Invalid rate");
      expect(() => calculateDailyChanges([{ rate: 0 }, { rate: 4.0 }])).toThrow(
        "Invalid rate",
      );
      expect(() =>
        calculateDailyChanges([{ rate: 4.0 }, { rate: NaN }]),
      ).toThrow("Invalid rate");
      expect(() =>
        calculateDailyChanges([{ rate: 4.0 }, { rate: undefined }]),
      ).toThrow("Invalid rate");
    });

    it("calculates percentage changes accurately with JS float protection", () => {
      const points = [{ rate: 4.0 }, { rate: 4.02 }, { rate: 4.0 }];
  ...
  describe("getHistogramDistribution", () => {
    it("integrates all steps correctly to output array sorted by min bin", async () => {
      ...
      const points = await fetchNbpRates(
        "USD",
        "2024-01-01",
        "2024-01-04",
        fetchMock,
      ...
      expect(result.find((b) => b.min === -2.0 && b.max === -1.5).count).toBe(
        1,
      );
