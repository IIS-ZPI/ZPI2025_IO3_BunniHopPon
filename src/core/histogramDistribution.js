export function calculateDailyChanges(points) {
  if (!Array.isArray(points) || points.length < 2) {
    throw new Error(
      "Insufficient points: at least two data points are required to calculate changes.",
    );
  }
  ...
  for (let i = 1; i < points.length; i++) {
    ...
    if (
      ...
      throw new Error(
        "Invalid rate: rates must be positive finite numbers and previous rate must be strictly greater than zero.",
      );
    }
